...
客户端可以直接伪造 `X-Real-IP` 和 `X-Forwarded-For` 请求头。只有当应用位于可信的反向代理之后，并且该代理会移除传入的转发请求头再自行设置这些请求头时，才能信任 `RemoteAddr()` 返回的地址。如果应用直接暴露在互联网上，请使用 `http.Request.RemoteAddr` 识别网络对端。请勿使用未经信任的地址进行访问控制等安全敏感的决策。
{{< /callout >}}
