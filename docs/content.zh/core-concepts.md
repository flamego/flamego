...
你可以为自己的返回值类型注册处理器。Flamego 会将路由处理器的返回值类型与已注册返回值处理器中非 `flamego.Context` 的参数进行匹配。
...
第一个返回值处理器会处理 `func() JSON`，第二个会处理 `func() (int, JSON)`。如果没有自定义返回值处理器匹配，Flamego 会回退到字符串、字节切片和错误的内置返回行为。
